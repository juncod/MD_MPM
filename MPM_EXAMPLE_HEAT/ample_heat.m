...
[lstps,g,mpData,mesh] = setupGrid_hole;                                          % setup information
...
nDoF = nodes;                                                            % total number of degrees of freedom
nmp  = length(mpData);                                                      % number of material points
lstp = 0;                                                                   % zero loadstep counter (for plotting function)

visualization(mpData, mesh);
run postPro;                                                                % plotting initial state & mesh

T_nodes = zeros(nDoF,1);

for lstp=1:lstps                                                            % loadstep loop
  fprintf(1,'\n%s %4i %s %4i\n','loadstep ',lstp,' of ',lstps);             % text output to screen (loadstep)
  [mesh,mpData] = elemMPinfo_Heat(mesh,mpData);                                  % material point - element information
  fext = detExtForce_Heat(nodes,mpData);                                    % external force calculation (total)
  ...
  dT_nodes  = zeros(nDoF,1);                                                     % zero the displacements
  fd   = detFDoFs_Heat(mesh);                                                    % free degrees of freedom
  ...
  while (fErr > tol) && (NRit < NRitMax) || (NRit < 2)                      % global equilibrium loop
    [ddT, drct] = linSolve_Heat(mesh.bc,Kt,oobf,NRit,fd);                        % linear solver
    T_nodes = T_nodes + ddT;                                                        % update displacements
    frct = frct+drct;                                                       % update reaction forces
    
    [fint,Kt,mpData] = detMPs_Heat(T_nodes,mpData,nD);                               % global stiffness & internal force
  ...
  mpData = updateMPs_Heat(T_nodes,mpData);                                           % update material points
